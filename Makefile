...
GO_DBG_BUILD = env CGO_CFLAGS='-O0 -g -DMDBX_BUILD_FLAGS_CONFIG="config.h"' go build -trimpath -tags=mdbx,debug -ldflags "-X main.gitCommit=${GIT_COMMIT} -X main.gitBranch=${GIT_BRANCH} -w" -gcflags=all="-N -l"  # see delve docs
...
mdbx-dbg:
	...
	@cd ethdb/mdbx/dist/ \
		...
		&& echo '#define MALLOC_CHECK_ 7' >> config.h \
		&& echo '#define MALLOC_PERTURB_ 42' >> config.h \
