...
func (c *LmdbCursor) putCurrent(k, v []byte) error {
	if c.bucketName == dbutils.PlainStateBucket {
		defer dbOpPutCurrentTimer.UpdateSince(time.Now())
	}
	return c.c.Put(k, v, lmdb.Current)
}
func (c *LmdbCursor) putNoOverwrite(k, v []byte) error {
	if c.bucketName == dbutils.PlainStateBucket {
		defer dbOpPutNoOverwriteTimer.UpdateSince(time.Now())
	}
	return c.c.Put(k, v, lmdb.NoOverwrite)
}
