...
func (opts MdbxOpts) Open() (RwKV, error) {
	...
	_ = env.SetDebug(mdbx.LogLvlTrace, mdbx.DbgAssert, mdbx.LoggerDoNotChange) // temporary disable error, because it works if call it 1 time, but returns error if call it twice in same process (what often happening in tests)
