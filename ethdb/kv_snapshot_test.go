...
func TestSnapshotGet(t *testing.T) {
	...
	kv := NewSnapshot2KV().SnapshotDB([]string{dbutils.HeaderPrefix}, sn1).SnapshotDB([]string{dbutils.BlockBodyPrefix}, sn2).DB(mainDB).MustOpen()
...
func TestSequence(t *testing.T) {
	mainDB := NewLMDB().InMem().MustOpen()
	db:=NewObjectDatabase(mainDB)
	v, err := db.Sequence(dbutils.EthTx, 10)
	t.Log(v,err)
	...
	for i:=index; i< index+10; i++ {
		...
		err = db.Append(dbutils.EthTx, b, []byte("some value"))
		if err!=nil {
			t.Log(err)
		}
	}

	t.Log(db.Sequence(dbutils.EthTx, 0))
	...
	for i:=index; i< index+10; i++ {
		b:=make([]byte, 8)
		binary.BigEndian.PutUint64(b, i)
		err = db.Append(dbutils.EthTx, b, []byte("some value2"))
		if err!=nil {
			t.Log(err)
		}
	}
	...
	db.Walk(dbutils.EthTx, []byte{}, 0, func(k, v []byte) (bool, error) {
		fmt.Println(common.Bytes2Hex(k), string(v))
		return true, nil
	})
}
...
func TestSnapshotSequence(t *testing.T) {
	mainDB := NewLMDB().InMem().MustOpen()
	snDB := NewLMDB().InMem().MustOpen()
	tx,err:=snDB.Begin(context.Background(), nil, RW)
	if err!=nil {
		t.Fatal(err)
	}
	size:=uint64(100)
	index,err:=tx.Sequence(dbutils.EthTx, size)
	if err!=nil {
		t.Fatal(err)
	}
	...
	for i:=index; i<index+size; i++ {
		...
		if err:=c.Append(key, []byte(fmt.Sprintf("snap %d", i))); err!=nil {
			tx.Rollback()
			t.Fatal(err)
		}
	}
	...
	kv:=NewSnapshot2KV().SnapshotDB([]string{dbutils.EthTx}, snDB).DB(mainDB).MustOpen()
	...
	for i:=index; i< index+10; i++ {
		b:=make([]byte, 8)
		binary.BigEndian.PutUint64(b, i)
		err = db.Append(dbutils.EthTx, b, []byte(fmt.Sprintf("db1 %d",i)))
		if err!=nil {
			t.Log(err)
		}
	}
	...
	for i:=index; i< index+10; i++ {
		b:=make([]byte, 8)
		binary.BigEndian.PutUint64(b, i)
		err = db.Append(dbutils.EthTx, b, []byte(fmt.Sprintf("db2 %d",i)))
		if err!=nil {
			t.Log(err)
		}
	}
