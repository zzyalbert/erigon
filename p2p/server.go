	...
	for {
		...
		case c := <-srv.checkpointAddPeer:
			...
			if err == nil {
				...
				p := srv.launchPeer(c)
				peers[c.node.ID()] = p
				srv.log.Debug("Adding p2p peer", "v", srv.Protocols[0].Version, "peercount", len(peers), "peerinbound", inboundCount, "id", p.ID(), "conn", c.flags, "addr", p.RemoteAddr(), "name", p.Name())
		...
		case pd := <-srv.delpeer:
			...
			delete(peers, pd.ID())
			srv.log.Debug("Removing p2p peer", "v", srv.Protocols[0].Version, "peercount", len(peers), "peerinbound", inboundCount, "id", pd.ID(), "duration", d, "req", pd.requested, "err", pd.err)
