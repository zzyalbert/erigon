...
func ComputeTxEnv(ctx context.Context, block *types.Block, cfg *params.ChainConfig, getHeader func(hash common.Hash, number uint64) *types.Header, checkTEVM func(common.Hash) (bool, error), engine consensus.Engine, dbtx ethdb.Tx, blockHash common.Hash, txIndex uint64) (core.Message, vm.BlockContext, vm.TxContext, *state.IntraBlockState, state.StateReader, error) {
	defer func(t time.Time) { fmt.Printf("tracing2.go:40: %s\n", time.Since(t)) }(time.Now())
	// Create the parent state database
	reader := state.NewCachedReader(state.NewPlainKvState(dbtx, block.NumberU64()-1), shards.NewStateCache(32, datasize.MB))
