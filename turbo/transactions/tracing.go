...
func ComputeTxEnv(ctx context.Context, block, parent *types.Block, cfg *params.ChainConfig, getHeader func(hash common.Hash, number uint64) *types.Header, checkTEVM func(common.Hash) (bool, error), engine consensus.Engine, dbtx ethdb.Tx, blockHash common.Hash, txIndex uint64) (core.Message, vm.BlockContext, vm.TxContext, *state.IntraBlockState, *state.PlainKVState, error) {
	defer func(t time.Time) { fmt.Printf("tracing5.go:40: %s\n", time.Since(t)) }(time.Now())
...
func TraceTx(
	...
	defer func(t time.Time) { fmt.Printf("tracing3.go:103: %s\n", time.Since(t)) }(time.Now())
	var (
		tracer vm.Tracer
		err    error
	)
